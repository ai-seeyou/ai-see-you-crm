...
import { coverageNote, structureNote } from "../lib/entities";
...
export default defineTool({
	...
	async execute({ contactId, threads }) {
		...
		const structure = history.employerStructure
			? ` ${structureNote(history.employerStructure)}`
			: "";
		const covers = coverageNote(history.coverage);

		return {
			found: true as const,
			...history,
			note: `${evidence}${reach}${structure}${covers ? ` ${covers}` : ""}`,
		};
	},
});
